...
body{background:var(--s1);background-image:radial-gradient(circle at 50% -20%,rgba(111,143,114,.055),transparent 38%)}

.hdr{height:60px;background:color-mix(in srgb,var(--s1) 82%,transparent);border-bottom-color:color-mix(in srgb,var(--border-h) 55%,transparent);transition:background var(--motion-base) var(--motion),border-color var(--motion-base) var(--motion)}
.hdr-in{max-width:1180px;padding:0 28px}
.brand-name{font-size:1rem;letter-spacing:-.035em}
...
.page{max-width:1120px;padding:92px 28px 56px}
.card{border-radius:14px;box-shadow:0 1px 1px rgba(0,0,0,.08);transition:border-color var(--motion-base) var(--motion),background var(--motion-base) var(--motion),box-shadow var(--motion-base) var(--motion)}
...
.landing-shell{min-height:100vh;display:grid;place-items:center;padding:88px 24px 56px}
...
.landing-kicker{font-size:10px;letter-spacing:.16em;font-weight:700;color:var(--t3)}
.landing-copy{max-width:650px;opacity:0;animation:pn-reveal .7s .06s var(--motion) forwards}
.landing-title{font-size:clamp(2.7rem,7vw,5.6rem);line-height:.98;letter-spacing:-.065em;font-weight:600;margin:0 0 24px;color:var(--t1)}
.landing-title em{font-style:normal;color:var(--t3);font-weight:450}
.landing-lede{max-width:540px;font-size:clamp(.95rem,1.7vw,1.08rem);line-height:1.7;color:var(--t2)}
...
.landing-url{font-family:'JetBrains Mono',monospace;font-size:clamp(.78rem,1.8vw,.9rem);color:var(--t3);padding:14px 0 22px;overflow:auto;white-space:nowrap}
.landing-url strong{color:var(--t1);font-weight:500}
...
.landing-step p{font-size:.86rem;color:var(--t2);line-height:1.5}
...
/* Quiet entry motion; no bounce, zoom or theatrics */
...
@media(max-width:640px){.hdr-in{padding:0 18px}.page{padding-left:18px;padding-right:18px}.landing-shell{padding:80px 20px 40px}.landing-brandline{margin-bottom:34px}.landing-title{font-size:clamp(2.55rem,13vw,4rem)}.landing-access{margin-top:40px}.landing-step{grid-template-columns:34px 1fr}}
`;
}
